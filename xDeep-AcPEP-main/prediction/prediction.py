...
def prediction(tissue_type, ad_param, temp_fea_file, model_path, result_root_folder, data_file_name):
        ...
        if pre_ad_filter.sum() != 0:
            ...
            # result_df.to_csv(result_root_folder + 'result_' + tissue_type + '_' +data_file_name.replace('.fasta','') + '.csv', index=False)
            result_df.to_csv(result_root_folder + data_file_name.replace('.fasta','') + '_' + tissue_type + '.csv', index=False)
...
if __name__ == '__main__':
    ...
    outputFolder = inputFolder + 'regression_result/'
    rootDir = os.path.abspath(os.getcwd())
    
    # create result folder
    if os.path.exists(inputFolder + 'regression_result') is False:
        os.mkdir(inputFolder + 'regression_result')
    else:
        os.system('rm -r ' + inputFolder + 'regression_result')
        os.mkdir(inputFolder + 'regression_result')
    
    for inputFile in glob.glob(inputFolder + '*_normal.fasta'):
        tissue_type = ['breast', 'cervix', 'colon', 'lung', 'prostate', 'skin']
        for eachTissue in tissue_type:
            # -t skin -m model/ -d $filename -o Anal_result/AnalTissues1DS_input_skin/
            tissue_type = eachTissue
            model_path = "xDeep-AcPEP-main/prediction/model/"
            fasta_data_file = inputFile
            result_root_folder = outputFolder 
